...
test('appearance presets persist default and custom values as one user choice', () => {
  const storage = memoryStorage()
  const service = new AppearanceService(storage)
  const profile = { mode: 'separate', unified: 100, typography: 115, width: 95, height: 105, verticalSpacing: 90, horizontalSpacing: 120 }
  ...
  assert.equal(service.get().uiScaleProfile.horizontalSpacing, 120)
})
...
test('named layout presets persist complete UI customization without changing business data', () => {
  const storage = memoryStorage()
  const service = new AppearanceService(storage)
  const profile = { mode: 'separate', unified: 100, typography: 110, width: 100, height: 100, verticalSpacing: 95, horizontalSpacing: 105 }
...
test('normalizeAppearancePatch accepts the new field and keeps legacy fields working', () => {
  ...
  assert.deepEqual(normalizeAppearancePatch({ uiScaleProfile: { mode: 'separate', unified: 100, typography: 115, width: 95, height: 105, verticalSpacing: 90, horizontalSpacing: 120 } }), { ok: true, patch: { uiScaleProfile: { mode: 'separate', unified: 100, typography: 115, width: 95, height: 105, verticalSpacing: 90, horizontalSpacing: 120 } } })
